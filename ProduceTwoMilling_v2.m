...
%             cen_loc(:,k,j,i) = curr_cen;
            ...
            temp_cen = [temp_cen curr_cen(1:3)];
            temp_cen_loc = [temp_cen_loc cen_cur(1:3)];
        ...
        figure(1)
        hold on
        color_cen = [0.2 0.2 0.7];
        pcl_cen = pointCloud(temp_cen','color',ones(length(temp_cen),1)*color_cen);
        pcshow(pcl_cen)
        hold on
        
        figure(2)
        
        pcl_cen = pointCloud(temp_cen_loc','color',ones(length(temp_cen_loc),1)*color_cen);
        pcshow(pcl_cen);
        hold on
...
%         phase_init = [phase_init, phase_rand];
        ...
        vp_init = rand(1);
        sp_pre = 0;
        vd_v = vd + 0.15 * sin(2*pi/sin_t*[1:numPointLine] + vp_init);
        vd_list = [vd_list; vd_v];
        
         for k = 1:numPointLine
            % at a certain position.
            clock = k/fr;
            sp_pre = sp_pre + vd_v(k)/fr;
            Vd = [sp_pre; w*(j-1); d*(i-1)];

            RZ = ROZ(clock*w_v + phase_rand);
            T_work = [RX*RY*RZ Vd; 0 0 0 1];
            PCL_cur = zeros(size(PCL));

            Ft = [vd_v(k)/num_teeth/rpm*60 0 0]';       % mm/teeth
...
%             sim_cen_loc(:,k,j,i) = curr_cen;
            sim_cen_loc(:,k,j,i) = TM_gray\curr_cen;
            ...
            temp_cen = [temp_cen curr_cen(1:3)];
            temp_temp_loc = TM_gray\curr_cen;
            temp_cen_loc = [temp_cen_loc temp_temp_loc(1:3)];
        end   % data of every trajectory is produced
        ...
        figure(1)
        hold on
        sim_color_cen = [0.8 0.2 0.3];
        sim_pcl_cen = pointCloud(temp_cen','color',ones(length(temp_cen),1)*sim_color_cen);
        pcshow(sim_pcl_cen);
        hold on

        figure(2)
        sim_pcl_cen = pointCloud(temp_cen_loc','color',ones(length(temp_cen_loc),1)*sim_color_cen);
        pcshow(sim_pcl_cen);
        hold on        
    end
end
...
save result_new1108.mat result_m sim_result cen_loc edge_loc sim_cen_loc sim_edge_loc tran_loc w d vd_list
...
plot(gray_mean,'r');
hold on
plot(sim_graygray_mean,'g');

plot(gray_max, 'r^')
plot(gray_min, 'ro')

plot(sim_graygray_max, 'g^')
plot(sim_graygray_min, 'go')
hold off
legend('Plan - GrayMean','Sim - GrayMean', 'Plan - MaxGray', 'Plan - MinGray',...
    'Sim - MaxGray', 'Sim - MinGray','location','eastoutside');
